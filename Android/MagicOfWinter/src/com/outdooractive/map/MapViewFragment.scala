...
class MapViewFragment extends SFragment with OnClickListener {
  ...
  override def onDestroyView() {
    super.onDestroyView
    val mapFragment = getFragmentManager().findFragmentById(R.id.map_fragment);
  ...
  private def map = Option(getFragmentManager().findFragmentById(R.id.map_fragment).asInstanceOf[SupportMapFragment].getMap)
